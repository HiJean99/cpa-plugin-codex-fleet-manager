...
type SettingsPayload struct {
	...
	ResetProbeModel                 string         `json:"reset_probe_model"`
	ResetProbeMode                  ResetProbeMode `json:"reset_probe_mode"`
	ResetProbeTimezone              string         `json:"reset_probe_timezone"`
	ResetProbeSchedule              string         `json:"reset_probe_schedule"`
	ResetProbeScheduleGrace         string         `json:"reset_probe_schedule_grace"`
	ResetProbeAllAccounts           bool           `json:"reset_probe_all_accounts"`
	ResetProbeRequireResetAtSlide   bool           `json:"reset_probe_require_reset_at_slide"`
	ResetProbeObservationInterval   string         `json:"reset_probe_observation_interval"`
	ResetProbeDriftThreshold        string         `json:"reset_probe_drift_threshold"`
	ResetProbeMinInterval           string         `json:"reset_probe_min_interval"`
	ResetProbeFailureCooldown       string         `json:"reset_probe_failure_cooldown"`
...
func SettingsFromConfig(cfg Config) SettingsPayload {
	return SettingsPayload{
		...
		ResetProbeModel:                 cfg.ResetProbeModel,
		ResetProbeMode:                  cfg.ResetProbeMode,
		ResetProbeTimezone:              cfg.ResetProbeTimezone,
		ResetProbeSchedule:              strings.Join(cfg.ResetProbeSchedule, ","),
		ResetProbeScheduleGrace:         cfg.ResetProbeScheduleGrace.String(),
		ResetProbeAllAccounts:           cfg.ResetProbeAllAccounts,
		ResetProbeRequireResetAtSlide:   cfg.ResetProbeRequireResetAtSlide,
		ResetProbeObservationInterval:   cfg.ResetProbeObservationInterval.String(),
		ResetProbeDriftThreshold:        cfg.ResetProbeDriftThreshold.String(),
		ResetProbeMinInterval:           cfg.ResetProbeMinInterval.String(),
		ResetProbeFailureCooldown:       cfg.ResetProbeFailureCooldown.String(),
...
func ConfigFromSettings(base Config, payload SettingsPayload) (Config, error) {
	...
	if payload.ResetProbeModel != "" {
		model, err := validateResetProbeModel(payload.ResetProbeModel)
		if err != nil {
			return Config{}, jsonError(err.Error())
		}
		cfg.ResetProbeModel = model
	}
	if payload.ResetProbeMode != "" {
		mode, err := validateResetProbeMode(string(payload.ResetProbeMode))
		if err != nil {
			return Config{}, jsonError(err.Error())
		}
		cfg.ResetProbeMode = mode
	}
	if payload.ResetProbeTimezone != "" {
		tz, err := validateResetProbeTimezone(payload.ResetProbeTimezone)
		if err != nil {
			return Config{}, jsonError(err.Error())
		}
		cfg.ResetProbeTimezone = tz
	}
	if payload.ResetProbeSchedule != "" {
		schedule, err := parseResetProbeSchedule(payload.ResetProbeSchedule)
		if err != nil {
			return Config{}, jsonError(err.Error())
		}
		cfg.ResetProbeSchedule = schedule
	}
	if payload.ResetProbeScheduleGrace != "" {
		d, err := time.ParseDuration(payload.ResetProbeScheduleGrace)
		if err != nil || d <= 0 {
			return Config{}, jsonError("reset_probe_schedule_grace must be a positive duration")
		}
		cfg.ResetProbeScheduleGrace = d
	}
	cfg.ResetProbeAllAccounts = payload.ResetProbeAllAccounts
	cfg.ResetProbeRequireResetAtSlide = payload.ResetProbeRequireResetAtSlide
	for _, value := range []struct {
		name string
		raw  string
		set  func(time.Duration)
	}{
		{"reset_probe_observation_interval", payload.ResetProbeObservationInterval, func(d time.Duration) { cfg.ResetProbeObservationInterval = d }},
		{"reset_probe_drift_threshold", payload.ResetProbeDriftThreshold, func(d time.Duration) { cfg.ResetProbeDriftThreshold = d }},
		{"reset_probe_min_interval", payload.ResetProbeMinInterval, func(d time.Duration) { cfg.ResetProbeMinInterval = d }},
		{"reset_probe_failure_cooldown", payload.ResetProbeFailureCooldown, func(d time.Duration) { cfg.ResetProbeFailureCooldown = d }},
	} {
		if value.raw == "" {
			continue
		}
		...
		if err != nil || d <= 0 {
			return Config{}, jsonError(value.name + " must be a positive duration")
		}
		value.set(d)
	}
...
<div class="warning" id="resetProbeWarning" hidden><strong data-i18n="resetProbe.warningTitle">自动激活新的额度周期默认关闭</strong><span data-i18n="resetProbe.warningBody">开启后，插件会按所选 5h 模式先读取额度；只有确认需要激活时，才发送一次极小的 Codex 请求。</span></div>
...
<div class="setting-with-help"><label class="toggle"><span data-i18n="settings.enableResetProbe">自动激活新的额度周期</span><input id="enableResetProbe" name="enable_reset_probe" type="checkbox"></label><p class="setting-help" data-i18n="settings.enableResetProbeHelp">resetAt 模式按观察间隔检测；固定时间表模式只在配置的时点检查。两种模式都会先读额度，只有确认需要激活时才发送极小请求。可能消耗少量额度。</p></div>
<label class="field"><span data-i18n="settings.resetProbeModel">自动激活模型</span><input id="resetProbeModel" spellcheck="false"></label>
<label class="field"><span data-i18n="settings.resetProbeMode">5h 激活模式</span><select id="resetProbeMode"><option value="reset_at" data-i18n="settings.resetProbeModeResetAt">按 resetAt 动态判断</option><option value="schedule" data-i18n="settings.resetProbeModeSchedule">按固定时间表</option></select></label>
<label class="field"><span data-i18n="settings.resetProbeTimezone">固定时间表时区</span><input id="resetProbeTimezone" spellcheck="false"></label>
<label class="field"><span data-i18n="settings.resetProbeSchedule">固定时间表</span><input id="resetProbeSchedule" spellcheck="false" placeholder="07:00,12:00,17:00"></label>
<label class="field"><span data-i18n="settings.resetProbeScheduleGrace">错过时点补触发宽限</span><input id="resetProbeScheduleGrace" spellcheck="false"></label>
<div class="setting-with-help"><label class="toggle"><span data-i18n="settings.resetProbeAllAccounts">固定时间表覆盖全部 Codex OAuth</span><input id="resetProbeAllAccounts" type="checkbox"></label><p class="setting-help" data-i18n="settings.resetProbeAllAccountsHelp">仅用于固定时间表模式。开启后，所有未禁用的 Codex OAuth（包括临时 cooldown/unavailable）都会独立暖窗；不会改变 CPA priority，也不会接管 fill-first。</p></div>
<div class="setting-with-help"><label class="toggle"><span data-i18n="settings.resetProbeRequireResetAtSlide">仅在 resetAt 向后滑动后激活（9Router 模式）</span><input id="resetProbeRequireResetAtSlide" type="checkbox"></label><p class="setting-help" data-i18n="settings.resetProbeRequireResetAtSlideHelp">仅用于 resetAt 模式。开启后，首次观察只记录 resetAt，后续 resetAt 滑动超过阈值才允许发送激活请求。</p></div>
<label class="field"><span data-i18n="settings.resetProbeObservationInterval">5h 探测观察间隔</span><input id="resetProbeObservationInterval" spellcheck="false"></label>
<label class="field"><span data-i18n="settings.resetProbeDriftThreshold">resetAt 滑动阈值</span><input id="resetProbeDriftThreshold" spellcheck="false"></label>
<label class="field"><span data-i18n="settings.resetProbeMinInterval">同账号最小激活间隔</span><input id="resetProbeMinInterval" spellcheck="false"></label>
<label class="field"><span data-i18n="settings.resetProbeFailureCooldown">激活失败冷却</span><input id="resetProbeFailureCooldown" spellcheck="false"></label>
...
const TRANSLATIONS={
  en:{
    ...
    'resetProbe.warningTitle':'Automatic reset probe is off by default','resetProbe.warningBody':'After opt-in, the plugin reads quota according to the selected 5h mode and sends one tiny Codex request only when activation is actually needed.',
    'settings.title':'Scheduler Settings','settings.summary':'Default configuration is ready; normally no manual changes are needed.','settings.handleEnabled':'Enable scheduler takeover','settings.usageFeedback':'Mark quota exhausted from failure feedback','settings.enableResetProbe':'Enable automatic reset probe','settings.enableResetProbeHelp':'resetAt mode observes quota at its configured interval; fixed-schedule mode checks only at configured wall-clock slots. Both modes read quota first and send a tiny request only when activation is needed. This may consume a small amount of quota.','settings.resetProbeModel':'Auto-ping model','settings.resetProbeMode':'5h activation mode','settings.resetProbeModeResetAt':'Dynamic resetAt','settings.resetProbeModeSchedule':'Fixed schedule','settings.resetProbeTimezone':'Schedule timezone','settings.resetProbeSchedule':'Fixed schedule','settings.resetProbeScheduleGrace':'Missed-slot catch-up grace','settings.resetProbeAllAccounts':'Warm all Codex OAuth accounts on schedule','settings.resetProbeAllAccountsHelp':'Schedule mode only. Every non-disabled Codex OAuth, including temporarily unavailable/cooldown accounts, gets its own warm window without changing CPA priority or taking over fill-first.','settings.resetProbeRequireResetAtSlide':'Require resetAt slide before activation (9Router mode)','settings.resetProbeRequireResetAtSlideHelp':'Used only in resetAt mode. When enabled, the first observation only records resetAt. An activation request is allowed only after resetAt slides by more than the configured threshold.','settings.resetProbeObservationInterval':'5h probe observation interval','settings.resetProbeDriftThreshold':'resetAt slide threshold','settings.resetProbeMinInterval':'Minimum activation interval per account','settings.resetProbeFailureCooldown':'Activation failure cooldown','settings.provisionalProbe':'Allow quota probes when the account roster is unconfirmed (high risk)','settings.provisionalProbeHelp':'When CPA temporarily cannot confirm the current accounts and priorities, allow the plugin to use the most recently saved account roster for quota reset probes. Account credentials are revalidated every time, but the plugin still cannot guarantee that accounts have not been removed or reprioritized. This should normally remain off.','settings.monthlyMode':'Monthly mode','settings.expiryOrder':'Sort by expiry time','settings.monthlyPriority':'Prefer Monthly','settings.refreshInterval':'Quota refresh interval','settings.staleAfter':'Stale cache threshold','settings.refreshActiveWindow':'Refresh active window','settings.refreshAfterResetDelay':'Refresh after reset delay','settings.refreshRetryDelays':'Refresh retry delays','settings.refreshOnStartup':'Refresh on startup','settings.maxConcurrency':'Max refresh concurrency','settings.circuitFailureThreshold':'Circuit failure threshold','settings.circuitOpenDuration':'Circuit open duration','settings.circuitHalfOpenSuccessThreshold':'Half-open recovery successes','settings.maxLogEntries':'Max log entries','settings.logRetention':'Log retention',
  ...
  'zh-CN':{
    'app.title':'Codex Fleet Manager','app.subtitle':'优化版 Fill First。配置、别名、分组、标签和备注由插件内部状态文件保存。','app.language':'界面语言','connection.managementKey':'CPA 管理密钥','connection.rememberManagementKey':'在此浏览器中记住管理密钥','connection.rememberManagementKeyHelp':'密钥将以未加密形式保存在浏览器本地存储中。请仅在受信任的设备上启用。','connection.backgroundHint':'只要调度器启动了，它就会在后台自动运行，无需保持页面开启。',
    'resetProbe.warningTitle':'自动激活新的额度周期默认关闭','resetProbe.warningBody':'开启后，插件会按所选 5h 模式先读取额度；只有确认需要激活时，才发送一次极小的 Codex 请求。',
    'settings.title':'调度设置','settings.summary':'默认配置已经都设置好了，正常情况下不需要手动设置。','settings.handleEnabled':'启用调度接管','settings.usageFeedback':'失败反馈标记额度耗尽','settings.enableResetProbe':'自动激活新的额度周期','settings.enableResetProbeHelp':'resetAt 模式按观察间隔检测；固定时间表模式只在配置的时点检查。两种模式都会先读额度，只有确认需要激活时才发送极小请求。可能消耗少量额度。','settings.resetProbeModel':'自动激活模型','settings.resetProbeMode':'5h 激活模式','settings.resetProbeModeResetAt':'按 resetAt 动态判断','settings.resetProbeModeSchedule':'按固定时间表','settings.resetProbeTimezone':'固定时间表时区','settings.resetProbeSchedule':'固定时间表','settings.resetProbeScheduleGrace':'错过时点补触发宽限','settings.resetProbeAllAccounts':'固定时间表覆盖全部 Codex OAuth','settings.resetProbeAllAccountsHelp':'仅用于固定时间表模式。开启后，所有未禁用的 Codex OAuth（包括临时 cooldown/unavailable）都会独立暖窗；不会改变 CPA priority，也不会接管 fill-first。','settings.resetProbeRequireResetAtSlide':'仅在 resetAt 向后滑动后激活（9Router 模式）','settings.resetProbeRequireResetAtSlideHelp':'仅用于 resetAt 模式。开启后，首次观察只记录 resetAt，后续 resetAt 滑动超过阈值才允许发送激活请求。','settings.resetProbeObservationInterval':'5h 探测观察间隔','settings.resetProbeDriftThreshold':'resetAt 滑动阈值','settings.resetProbeMinInterval':'同账号最小激活间隔','settings.resetProbeFailureCooldown':'激活失败冷却','settings.provisionalProbe':'账号列表未确认时仍允许额度探测（高风险）','settings.provisionalProbeHelp':'CPA 暂时无法确认当前账号及优先级时，允许插件使用最近一次保存的账号列表执行额度重置探测。每次都会重新验证账号凭据，但仍无法保证账号未被删除或调整优先级。通常应保持关闭。','settings.monthlyMode':'月度账号使用方式','settings.expiryOrder':'按到期时间排序','settings.monthlyPriority':'优先使用月度账号','settings.refreshInterval':'额度刷新间隔','settings.staleAfter':'缓存过期判定','settings.refreshActiveWindow':'活跃刷新窗口','settings.refreshAfterResetDelay':'重置后刷新延迟','settings.refreshRetryDelays':'刷新失败重试间隔','settings.refreshOnStartup':'启动时刷新额度','settings.maxConcurrency':'最大并发刷新','settings.circuitFailureThreshold':'熔断失败阈值','settings.circuitOpenDuration':'熔断等待时间','settings.circuitHalfOpenSuccessThreshold':'半开恢复成功次数','settings.maxLogEntries':'最大日志条数','settings.logRetention':'日志保留时间',
...
function collectSettingsPayload(){return{handle_enabled:document.getElementById('handleEnabled').checked,enable_usage_feedback:document.getElementById('usageFeedback').checked,enable_reset_probe:document.getElementById('enableResetProbe').checked,reset_probe_model:document.getElementById('resetProbeModel').value.trim(),reset_probe_mode:document.getElementById('resetProbeMode').value,reset_probe_timezone:document.getElementById('resetProbeTimezone').value.trim(),reset_probe_schedule:document.getElementById('resetProbeSchedule').value.trim(),reset_probe_schedule_grace:document.getElementById('resetProbeScheduleGrace').value.trim(),reset_probe_all_accounts:document.getElementById('resetProbeAllAccounts').checked,reset_probe_require_reset_at_slide:document.getElementById('resetProbeRequireResetAtSlide').checked,reset_probe_observation_interval:document.getElementById('resetProbeObservationInterval').value.trim(),reset_probe_drift_threshold:document.getElementById('resetProbeDriftThreshold').value.trim(),reset_probe_min_interval:document.getElementById('resetProbeMinInterval').value.trim(),reset_probe_failure_cooldown:document.getElementById('resetProbeFailureCooldown').value.trim(),probe_on_provisional_roster:document.getElementById('probeOnProvisionalRoster').checked,monthly_mode:document.getElementById('monthlyMode').value,quota_refresh_interval:document.getElementById('refreshInterval').value.trim(),stale_after:document.getElementById('staleAfter').value.trim(),refresh_active_window:document.getElementById('refreshActiveWindow').value.trim(),refresh_after_reset_delay:document.getElementById('refreshAfterResetDelay').value.trim(),refresh_retry_delays:document.getElementById('refreshRetryDelays').value.trim(),refresh_on_startup:document.getElementById('refreshOnStartup').checked,max_refresh_concurrency:Number.parseInt(document.getElementById('maxConcurrency').value,10)||1,circuit_failure_threshold:Number.parseInt(document.getElementById('circuitFailureThreshold').value,10)||5,circuit_open_duration:document.getElementById('circuitOpenDuration').value.trim(),circuit_half_open_success_threshold:Number.parseInt(document.getElementById('circuitHalfOpenSuccessThreshold').value,10)||2,max_log_entries:Number.parseInt(document.getElementById('maxLogEntries').value,10)||200,log_retention:document.getElementById('logRetention').value.trim()}}
...
function fillSettings(){const s=STATUS.settings||{};document.getElementById('handleEnabled').checked=s.handle_enabled!==false;document.getElementById('usageFeedback').checked=s.enable_usage_feedback!==false;document.getElementById('enableResetProbe').checked=s.enable_reset_probe===true;document.getElementById('resetProbeModel').value=s.reset_probe_model||'gpt-5.6-terra';document.getElementById('resetProbeMode').value=s.reset_probe_mode||'reset_at';document.getElementById('resetProbeTimezone').value=s.reset_probe_timezone||'Asia/Shanghai';document.getElementById('resetProbeSchedule').value=s.reset_probe_schedule||'07:00,12:00,17:00';document.getElementById('resetProbeScheduleGrace').value=s.reset_probe_schedule_grace||'15m0s';document.getElementById('resetProbeAllAccounts').checked=s.reset_probe_all_accounts===true;document.getElementById('resetProbeRequireResetAtSlide').checked=s.reset_probe_require_reset_at_slide===true;document.getElementById('resetProbeObservationInterval').value=s.reset_probe_observation_interval||'30m0s';document.getElementById('resetProbeDriftThreshold').value=s.reset_probe_drift_threshold||'2m0s';document.getElementById('resetProbeMinInterval').value=s.reset_probe_min_interval||'10m0s';document.getElementById('resetProbeFailureCooldown').value=s.reset_probe_failure_cooldown||'10m0s';document.getElementById('probeOnProvisionalRoster').checked=s.probe_on_provisional_roster===true;document.getElementById('monthlyMode').value=s.monthly_mode||'expiry_order';document.getElementById('refreshInterval').value=s.quota_refresh_interval||'30m0s';document.getElementById('staleAfter').value=s.stale_after||'5h0m0s';document.getElementById('refreshActiveWindow').value=s.refresh_active_window||'1h0m0s';document.getElementById('refreshAfterResetDelay').value=s.refresh_after_reset_delay||'1m0s';document.getElementById('refreshRetryDelays').value=s.refresh_retry_delays||'1m0s,5m0s,15m0s';document.getElementById('refreshOnStartup').checked=s.refresh_on_startup===true;document.getElementById('maxConcurrency').value=s.max_refresh_concurrency||1;document.getElementById('circuitFailureThreshold').value=s.circuit_failure_threshold||5;document.getElementById('circuitOpenDuration').value=s.circuit_open_duration||'30m0s';document.getElementById('circuitHalfOpenSuccessThreshold').value=s.circuit_half_open_success_threshold||2;document.getElementById('maxLogEntries').value=s.max_log_entries||200;document.getElementById('logRetention').value=s.log_retention||'24h0m0s'}
